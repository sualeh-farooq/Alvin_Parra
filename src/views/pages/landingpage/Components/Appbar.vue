...
<template>
    ...
    <v-container class="fill-height maxWidth">
      <div class="d-flex align-center gap-2 w-100">
        ...
        <template v-if="mdAndUp">
          ...
          <v-btn variant="flat" color="secondary" target="_" href="https://store.vuetifyjs.com/products/berry-vuetify-admin-template">Purchase Now</v-btn>
        </template>
        <template v-else>
  ...
  <v-navigation-drawer v-model="drawer" temporary location="right" floating v-if="!mdAndUp">
    <v-list color="primary">
      ...
      <v-list-item to="https://codedthemes.gitbook.io/berry-vuetify/">
        <template v-slot:prepend>
      ...
      <v-list-item to="https://vuetifyjs.com">
        <template v-slot:prepend>
          <CreditCardIcon size="20" stroke-width="1.5px" />
        </template>

        <v-list-item-title class="ml-3" to="https://store.vuetifyjs.com/products/berry-vuetify-admin-template">Purchase Now</v-list-item-title>
      </v-list-item>
    </v-list>
  </v-navigation-drawer>
</template>
