...
mock.onPost('/api/mails/setRead').reply((config:any) => {
  try {
...
mock.onPost('/api/mails/setImportant').reply((config:any) => {
  try {
    const { id } = JSON.parse(config.data);
    const mailIndex = mails.findIndex((i) => i.id === id);
...
mock.onPost('/api/mails/setStarred').reply((config:any) => {
  try {
    const { id } = JSON.parse(config.data);
    const mailIndex = mails.findIndex((i) => i.id === id);
...
mock.onPost('/api/mails/filter').reply((config:any) => {
  try {
