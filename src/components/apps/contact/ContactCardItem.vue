...
<template>
  <v-row>
    <v-col class="mt-3" :lg="typeof contact === 'string' ? '12' : '3'" v-for="contact in getContacts" :key="contact.name">
      <h3 v-if="typeof contact === 'string'" class="text-primary text-h3 mt-5">{{ contact }}</h3>
          ...
          <div class="d-flex">
            ...
            <div class="ml-auto">
              <v-menu class="rounded-md">
                ...
                <v-list>
                  <v-list-item value="Edit" active-color="secondary" @click="$emit('openDrawer'), store.SelectContact(contact.id)">
