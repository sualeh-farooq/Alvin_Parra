<script setup lang="ts">
import { ref } from 'vue';
const props = defineProps({
  task: Object
});
const items = ref([
  {
    title: 'Edit'
  },
  {
    title: 'Delete'
  }
]);
</script>
<template>
  <v-card variant="outlined" class="withbg cursor-move">
    <v-card-text class="pa-4">
      <div class="d-flex align-start gap-2">
        <div class="d-block text-truncate">
          <h5 class="text-h5 text-truncate">{{ task?.title }}</h5>
          <a class="text-secondary text-subtitle-2 d-flex align-center cursor-pointer">
            <NotebookIcon size="14" /> User Story #{{ task?.userStory }}
          </a>
        </div>
        <v-btn icon size="small" variant="text" class="ml-auto">
          <DotsVerticalIcon size="14" />
          <v-menu activator="parent" class="rounded-md">
            <v-list>
              <v-list-item v-for="(item, index) in items" :key="index" :value="index">
                <v-list-item-title>{{ item.title }}</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-menu>
        </v-btn>
      </div>
      <img v-if="task?.image" :src="task?.image" class="w-100 mt-3 rounded-sm" alt="Avatar" />
    </v-card-text>
  </v-card>
</template>
<style>
.cursor-move {
  cursor: move;
}
</style>
