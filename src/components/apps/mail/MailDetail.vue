...
<template>
  <div class="topbarMail d-flex gap-3 align-center w-100">
    <v-btn icon flat @click="$emit('toggleDetail')" size="small"><ChevronLeftIcon size="18" /></v-btn>
    <div class="d-flex align-center gap-3 w-100">
      <img :src="selectedMail?.profile.avatar" :alt="selectedMail?.profile.avatar" width="40" />
      <div>
        <h4 class="text-h4 mb-n1">{{ selectedMail?.profile.name }}</h4>
        <small>From: {{ selectedMail?.profile.email }}</small>
      </div>
  ...
  <div class="py-3">
    ...
    <v-row class="mt-4">
      <v-col cols="3" v-for="attach in selectedMail?.attachments">
