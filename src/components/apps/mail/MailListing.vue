...
const getMails: any = computed(() => {
  return store.mails;
});

const searchValue = ref('');
const compact = ref(false);
const sorting = ref([{ title: 'Name' }, { title: 'Date' }, { title: 'Rating' }, { title: 'Unread' }]);
...
const handleStarredChange = async (id: any) => {
  if (id) {
...
const countSelectedMail = computed(() => {
  return getMails.value.filter((i: any) => i.isChecked).length;
});

const handleUserChange = async (data: any | null) => {
  await store.setRead(data.id);
  handleFilter(store.activeFilter);
  emailDetails.value = true;
  selectedMail.value = data;
};
const filteredMails = computed(() => {
  return getMails.value.filter((mail: any) => {
...
<template>
  <div v-if="!emailDetails">
    ...
    <div class="pa-3 d-flex align-center">
      <v-btn icon size="small" variant="text" @click="$emit('sToggle')"><Menu2Icon size="18" /></v-btn>
      ...
      <div class="ml-auto">
        <v-text-field
          ...
          persistent-placeholder placeholder="Search Mail"
          hide-details
          density="compact"
          class="max-width-300"
        ></v-text-field>
